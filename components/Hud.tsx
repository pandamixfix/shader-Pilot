...
export const Hud: React.FC = () => {
    const { cameraRef, renderCameraRef, cameraVelocityRef, isHudEnabled, collisionState, viewModeTransition, playerStats } = useAppContext();
    ...
    useEffect(() => {
        ...
        const render = () => {
            ...
            // Damage Overlay (Red Tint if HP low or colliding)
            const stats = playerStatsRef.current;
            const hpPct = stats.currentHp / stats.maxHp;
            if (hpPct < 0.3 || currentState === 'colliding') {
                const intensity = currentState === 'colliding' ? 0.3 : (0.3 - hpPct) * 1.5;
            ...
            ctx.strokeStyle = `rgba(${baseColorStr}, 0.5)`;
            ctx.strokeRect(barX, barY, barW, barH);
            
            // Fill
            const fillW = Math.max(0, (stats.currentHp / stats.maxHp) * barW);
            const fillCol = stats.currentHp < stats.maxHp * 0.3 ? 'rgb(255, 50, 50)' : 'rgb(0, 255, 255)';
            ...
            ctx.fillText(`HP: ${Math.ceil(stats.currentHp)} / ${stats.maxHp}`, cx, barY - 5);

            // --- STATUS READOUTS (Always visible) ---
            const vel = cameraVelocityRef.current;
            const speed = Math.sqrt(vel[0]*vel[0] + vel[1]*vel[1] + vel[2]*vel[2]);
            const speedDisplay = (speed * 100).toFixed(0);
            const altDisplay = altitude.toFixed(0);
            
            ctx.font = 'bold 16px monospace';
            ctx.fillStyle = 'rgba(255, 255, 255, 0.9)';
            ctx.shadowColor = 'rgba(0, 0, 0, 0.5)';
            ctx.shadowBlur = 4;
            ctx.fillText(`СКР: ${speedDisplay}   ВЫС: ${altDisplay}`, cx, barY - 25);
            ctx.shadowBlur = 0;
            ...
            if (hudAlpha > 0.01) {
                ...
                // --- READOUTS ---
                const altText = `ВЫС: ${altitude.toFixed(2)}`;
                const vsText = `В/С: ${(vY * 10).toFixed(2)}`;
