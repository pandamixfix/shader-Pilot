...
const getAiStatusText = (stage: AiStage): string | null => {
    switch (stage) {
        case AiStage.ADJUSTING_SLIDERS:
            return 'AI настраивает ползунки...';
        case AiStage.SMART_SLIDER_CREATION:
            return 'AI создает новый контроль...';
        case AiStage.MODIFYING_CODE:
            return 'AI модифицирует код шейдера...';
        case AiStage.ENABLE_CAMERA_CONTROLS:
            return 'AI добавляет управление камерой...';
        default:
            return null;
    }
}
    ...
    >
      <div className={`overflow-hidden flex flex-col h-full ${isVisible ? 'min-w-[300px] lg:min-w-[450px]' : 'min-w-0'}`}>
        <div className="flex-shrink-0 flex items-center justify-between p-2 lg:hidden border-b border-gray-700">
          <h2 className="text-lg font-semibold text-gray-200">Редактор</h2>
          <button 
        ...
        <div className="flex-grow overflow-y-auto">
              ...
              <div className="flex justify-between items-center mb-2">
                ...
                {selectedSnippet && (
                    ...
                    >
                        ...
                        {isGeneratingExplanation ? 'Анализ...' : 'Объяснить код'}
                    </button>
                )}
              </div>
              ...
              {error ? (
                <div className="bg-red-900/50 border border-red-700 text-red-300 text-xs font-mono p-3 rounded-md whitespace-pre-wrap max-h-48 overflow-y-auto">
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="font-bold mb-1 text-red-200">Ошибка компиляции:</p>
                      {error}
                    </div>
                    <button
                    ...
                    >
                      ...
                      {isFixingCode ? 'Исправление...' : 'Исправить (AI)'}
                    </button>
                  </div>
                </div>
                  ...
                  <div className="relative bg-gray-800/50 border border-gray-600 p-3 rounded-md text-sm">
                      ...
                      {isGeneratingExplanation && !explanation && !explanationError && (
                          <p className="text-gray-300 animate-pulse">Думаю...</p>
                      )}
                      {explanationError && (
                          <div className="text-red-400">
                              <p><span className="font-bold">Ошибка:</span> {explanationError}</p>
                          </div>
                      )}
                      {explanation && (
            ...
            <div className="p-4 border-t border-gray-800 space-y-4">
                <div className="flex justify-between items-center">
                    <h3 className="text-sm font-medium text-gray-300">Управление шейдером</h3>
                    {sliders.length > 0 && (
                        <button 
                            onClick={onResetSliders}
                            className="flex items-center gap-1.5 text-xs text-gray-400 hover:text-white transition-colors"
                            title="Удалить все ползунки"
                        >
                ...
                <div className="space-y-2 pt-2 border-t border-gray-800">
                    ...
                    >
                        ...
                        {isFetchingSuggestions ? 'Поиск идей...' : 'Предложить настройки'}
                    </button>

                    {suggestionsError && (
                        <div className="text-red-400 text-xs p-2 rounded-md bg-red-900/50 border border-red-700">
                            <p><span className="font-bold">Ошибка предложений:</span> {suggestionsError}</p>
                        </div>
                    )}
                    ...
                    {sliderSuggestions.length > 0 && !isFetchingSuggestions && (
                        <div className="pt-2">
                            <div className="flex items-center justify-between mb-2">
                                <p className="text-xs text-gray-400">Нажмите, чтобы применить с AI:</p>
                                <button onClick={onClearSuggestions} className="text-xs text-gray-500 hover:text-white px-2 py-1 rounded-md hover:bg-gray-700 transition-colors">&times; Очистить</button>
                            </div>
                ...
                {isAnalyzing && (
                    <p className="text-sm text-gray-400 animate-pulse text-center py-2">
                        {sliders.length > 0 ? 'Улучшаю описания...' : 'Анализ шейдера...'}
                    </p>
                )}

                {analysisError && (
                    <div className="text-red-400 text-xs p-2 rounded-md bg-red-900/50 border border-red-700">
                        <p><span className="font-bold">Ошибка анализа:</span> {analysisError}</p>
                    </div>
                )}
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <div className="flex items-center opacity-0 group-hover:opacity-100 focus-within:opacity-100 transition-opacity">
                                            <button
                                                ...
                                                title={isEditing ? "Завершить" : "Изменить диапазон"}
        ...
        <div className="p-4 bg-gray-950/50 border-t border-gray-700">
          <div className="relative space-y-3 mb-4">
              ...
              <input 
                ...
                placeholder="напр., сделай синее свечение"
              ...
              <div className="h-4 text-xs text-center text-gray-400">
                ...
                {geminiError && (
                <div className="text-red-400 text-xs p-2 rounded-md bg-red-900/50 border border-red-700">
                  <p><span className="font-bold">AI Ошибка:</span> {geminiError}</p>
                </div>
              )}
          </div>
          <div className="flex gap-2">
            <button
                ...
                title="Использовать AI для создания нового кода или контроллеров"
            >
                <SparklesIcon className={`text-xl ${aiStage !== AiStage.ADJUSTING_SLIDERS && isGenerating ? 'animate-spin' : ''}`} />
                {aiStage !== AiStage.ADJUSTING_SLIDERS && isGenerating ? 'Думаю...' : 'Создать'}
            </button>
             {sliders.length > 0 && (
                <button
                    ...
                    title="Использовать AI для настройки существующих ползунков"
                >
                    <AdjustmentsIcon className={`text-xl ${aiStage === AiStage.ADJUSTING_SLIDERS ? 'animate-spin' : ''}`} />
                    {aiStage === AiStage.ADJUSTING_SLIDERS ? 'Настраиваю...' : 'Настроить'}
                </button>
            )}
          </div>
        </div>
