...
const AppContent: React.FC = () => {
    const {
        ...
        isControlsOpen, // Used to pause ShaderCanvas
        setIsControlsOpen,
        isHdEnabled,
        setIsHdEnabled,
        isFpsEnabled,
    ...
    // Pause shader animations when menu is open
    const isPlaying = !isControlsOpen;
        ...
        <div className="h-screen w-screen bg-gray-900 text-white flex flex-col overflow-hidden relative">
            ...
            <div className="fixed top-4 left-4 z-30 flex flex-col gap-2">
                <button
                    ...
                    aria-label={`HD Режим (${isHdEnabled ? 'Вкл' : 'Выкл'})`}
                    title={`HD Режим (${isHdEnabled ? 'Вкл' : 'Выкл'})`}
                >
                 ...
                 {cameraControlsEnabled && SHOW_HUD_BUTTON && (
                    <button
                        ...
                        aria-label={`Переключить вид (Текущий: ${viewMode})`}
                        title={viewMode === 'chase' ? "Вид из кабины" : "Вид от третьего лица"}
                    >
            ...
            <div className="fixed top-4 right-4 z-30 flex flex-col gap-2">
                ...
                {SHOW_MUTE_BUTTON && (
                    <button
                        ...
                        title={`Звук: ${!soundConfig.enabled ? 'Выкл' : 'Вкл'}`}
                    >
                        {getVolumeIcon()}
                    </button>
                )}
